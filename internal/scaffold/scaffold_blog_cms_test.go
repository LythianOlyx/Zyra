...
func TestGenerate_BlogCmsBuildsAsStandaloneModule(t *testing.T) {
	if testing.Short() {
	...
	dest := filepath.Join(t.TempDir(), "standalone-blog-cms-app")
	res, err := Generate(Options{
		AppName:              "Standalone Blog CMS App",
		Template:             "blog-cms",
		Dest:                 dest,
	...
	mustExist := []string{
		...
		"content/posts.go", "content/posts_test.go",
		"pages/index.tsx", "pages/blog/[slug].tsx",
