...
func TestGenerate_RealtimeCollabBuildsAsStandaloneModule(t *testing.T) {
	if testing.Short() {
	...
	dest := filepath.Join(t.TempDir(), "standalone-realtime-collab-app")
	res, err := Generate(Options{
		AppName:              "Standalone Realtime Collab App",
		Template:             "realtime-collab",
		Dest:                 dest,
	...
	mustExist := []string{
		...
		"actions/board.go", "actions/board_test.go",
		"pages/index.tsx",
