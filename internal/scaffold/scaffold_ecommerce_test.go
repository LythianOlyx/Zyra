...
func TestGenerate_EcommerceBuildsAsStandaloneModule(t *testing.T) {
	if testing.Short() {
	...
	dest := filepath.Join(t.TempDir(), "standalone-ecommerce-app")
	res, err := Generate(Options{
		AppName:              "Standalone ECommerce App",
		Template:             "ecommerce",
		Dest:                 dest,
	...
	mustExist := []string{
		...
		"actions/catalog.go", "actions/catalog_test.go",
		"pages/index.tsx", "pages/cart.tsx", "pages/admin/products.tsx",
