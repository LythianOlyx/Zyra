...
func registerActions(reg *zyra.ActionRegistry) {
	reg.Register("actions", "SubmitContactForm", func(ctx context.Context, payload []byte) (interface{}, error) {
		var input actions.ContactFormInput
		if len(payload) > 0 {
			_ = json.Unmarshal(payload, &input)
		}
		return actions.SubmitContactForm(ctx, input)
	})
}
